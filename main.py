...
# ---------- TITLE ----------
st.markdown("<h1 style='text-align:center; color:#145A32'>Dashboard Analisis GNN & Clustering Peserta Tuberculosis BPJS Kesehatan Jakarta</h1>", unsafe_allow_html=True)

# ================== LOAD DATA ==================
df_vis = load_data_visualization()
node_peserta, node_faskes, edges = load_data_gnn()

# Pastikan kolom string untuk sinkronisasi
df_vis['peserta_id'] = df_vis['peserta_id'].astype(str)
node_peserta['peserta_id'] = node_peserta['peserta_id'].astype(str)
edges['peserta_id'] = edges['peserta_id'].astype(str)
edges['faskes_id'] = edges['faskes_id'].astype(str)
node_faskes['faskes_id'] = node_faskes['faskes_id'].astype(str)

# ================== K-MEANS ==================
embedding_cols = [col for col in ['Usia', 'Jarak_km', 'Biaya_Klaim'] if col in node_peserta.columns]
if embedding_cols:
    embeddings = node_peserta[embedding_cols].fillna(0).values
else:
    embeddings = np.random.rand(len(node_peserta),2)
...
def detect_anomalies(node_peserta, embedding_cols, threshold=3.0):
    ...
    distances = []
    for idx, row in node_peserta.iterrows():
        cluster = row['k-means_cluster']
        centroid = centroids.loc[cluster].values
        dist = np.linalg.norm(row[embedding_cols].values - centroid)
...
edges_filtered = edges_clustered[edges_clustered['peserta_id'].isin(node_peserta_filtered['peserta_id'])]
...
if selected_anomali != "All":
    if selected_anomali == "Anomali":
        node_peserta_filtered = node_peserta_filtered[node_peserta_filtered['anomali']==True]
    else:
        node_peserta_filtered = node_peserta_filtered[node_peserta_filtered['anomali']==False]
    df_filtered = df_filtered[df_filtered['peserta_id'].isin(node_peserta_filtered['peserta_id'])]
...
merged['Biaya_Klaim'] = pd.to_numeric(merged['Biaya_Klaim'], errors='coerce').fillna(0)
...
with col1:
    if 'jenis_kelamin' in df_filtered.columns and total_peserta>0:
        ...
        fig_gender = px.pie(
            gender_count, names='Jenis Kelamin', values='Jumlah', hole=0.4,
...
with col2:
    if 'Segmentasi' in df_filtered.columns and total_peserta>0:
        ...
        fig_seg = px.pie(
            seg_count, names='Segmentasi', values='Jumlah', hole=0.4,
...
if 'Usia' in df_filtered.columns and total_peserta>0:
    fig_age = px.histogram(df_filtered, x='Usia', nbins=20, title="Distribusi Usia Peserta",
...
if total_peserta>0 and ('Biaya_Klaim' in df_filtered.columns or 'rata-rata_klaim_biaya' in df_filtered.columns):
    col_cost = 'rata-rata_klaim_biaya' if 'rata-rata_klaim_biaya' in df_filtered.columns else 'Biaya_Klaim'
    fig_cost = px.histogram(df_filtered, x=col_cost, nbins=50, title="Distribusi Biaya Klaim",
                            color_discrete_sequence=['#1ABC9C'])
...
if total_peserta>0 and 'Usia' in node_peserta_filtered.columns and 'k-means_cluster' in node_peserta_filtered.columns:
...
if total_peserta>0:
    ...
    fig_cluster.update_layout(title_font_color='#145A32', xaxis_title="Cluster", yaxis_title="Jumlah Peserta")
...
if len(anomali_df) > 0:
    ...
    st.write("Tidak ada peserta yang terindikasi anomali.")
